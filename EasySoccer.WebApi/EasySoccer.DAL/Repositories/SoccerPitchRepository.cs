...
namespace EasySoccer.DAL.Repositories
{
    public class SoccerPitchRepository : RepositoryBase, ISoccerPitchRepository
    {
        ...
        {
            return _dbContext.SoccerPitchQuery.Where(x => x.CompanyId == company && x.Active).OrderBy(x => x.Name).ToListAsync();
        }
        ...
        public Task<SoccerPitch> GetAsync(long id, bool includeInfo = false)
        {
            ...
            if (includeInfo)
                query = query.Include(x => x.SoccerPitchSoccerPitchPlans).Include(x => x.SportType).Include("SoccerPitchSoccerPitchPlans.SoccerPitchPlan");
