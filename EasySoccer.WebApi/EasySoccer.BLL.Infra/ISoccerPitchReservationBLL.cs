...
namespace EasySoccer.BLL.Infra
{
    public interface ISoccerPitchReservationBLL
    {
        ...
        Task<SoccerPitchReservationResponse> CreateAsync(long soccerPitchId, Guid? personId, DateTime selectedDate, TimeSpan hourStart, TimeSpan hourFinish, string note, long? companyUserId, long soccerPitchPlanId, Guid? personCompanyId, ApplicationEnum application);
        Task<SoccerPitchReservation> UpdateAsync(Guid id, long soccerPitchId, Guid? personId, DateTime selectedDate, TimeSpan hourStart, TimeSpan hourFinish, string note, long soccerPitchSoccerPitchPlanId, Guid? personCompanyId);
        ...
        Task<List<SoccerPitchReservation>> GetReservationsByMonthOrDay(int month, int? day, long companyId, int year, List<long> soccerPitches, List<int> status);
        ...
        Task<decimal> GetReservationValueAsync(Guid soccerPitchReservationId);
        Task<bool> ChangeStatusAllReservationsAsync(Guid reservationId);
    }
}
